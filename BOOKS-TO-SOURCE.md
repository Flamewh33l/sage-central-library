# Books to Source - Alexandria Curriculum
...
### Definite Stubs (under 5KB)
These files are too small to be real books:

| File | Size | Issue |
|------|------|-------|
| bob_proctor_art_of_living.pdf | 146 bytes | Empty/stub |
| bob_proctor_you_were_born_rich.pdf | 1,678 bytes | Stub |
| cs_lewis_mere_christianity.pdf | 1,667 bytes | Stub |
| thich_nhat_hanh_peace_is_every_step.pdf | 1,667 bytes | Suspicious (same size as C.S. Lewis) |
| robert_greene_48_laws_of_power.pdf | 1,678 bytes | Suspicious (same size as Bob Proctor) |
| isaac_asimov_foundation.pdf | 4,056 bytes | Stub |

### Suspicious 155KB Cluster (likely placeholders/excerpts)
All ~155,485-155,488 bytes — too uniform to be real full texts:

| File | Expected Size |
|------|---------------|
| james_clear_atomic_habits.pdf | Should be ~300KB+ |
| albert_einstein_relativity.pdf | Should be ~200KB+ |
| gary_chapman_five_love_languages.pdf | Should be ~250KB+ |
| george_rr_martin_a_game_of_thrones.pdf | Should be 1MB+ |
| martin_heidegger_being_and_time.pdf | Should be 500KB+ |
| peter_lynch_one_up_on_wall_street.pdf | Should be ~400KB+ |
| roland_barthes_pleasure_of_text.pdf | Should be ~200KB+ |
| chris_macdonald_ethics.pdf | Unknown |
| dave_ramsey_total_money_makeover.pdf | Should be ~400KB+ |
| david_mcnally_global_slavery.pdf | Unknown |
| gordon_marino_rethinking_nietzsche.pdf | Should be ~300KB+ |
| isaac_newton_mathematical_principles.pdf | Should be 500KB+ |
| jrr_tolkien_lord_of_rings.pdf | Should be 1MB+ |
| karl_popper_open_society.pdf | Should be 600KB+ |
| lewis_vaughn_philosophy.pdf | Should be ~300KB+ |
| paul_podolsky_ultimate_self_help_guide.pdf | Unknown |

### Other Suspicious Files
| File | Size | Issue |
|------|------|-------|
| suzanne_collins_hunger_games.pdf | 146,798 bytes | Small for a novel |
| william_golding_lord_of_the_flies.pdf | 65MB | Scanned images, no text layer |

---

## Blocked Authors (Flame will source)

Five authors whose works need alternative sourcing:

1. **J.K. Rowling** — Harry Potter series
2. **Dean Koontz** — Various thrillers
3. **Thomas Harris** — Hannibal Lecter series
4. **Ryu Murakami** — Japanese literature
5. **Robert Nathan** — Unknown specific works

---

## Notes

- Lord of the Flies needs OCR processing or text-based replacement (currently scanned images only)
- Multiple files share identical sizes (1,667 bytes, 1,678 bytes) suggesting batch stub creation
- 155KB cluster suggests automated placeholder generation

---

*Update this file as books are sourced and replaced.*
